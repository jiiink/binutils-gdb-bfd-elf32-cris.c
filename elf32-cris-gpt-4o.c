...
static reloc_howto_type *cris_reloc_type_lookup(bfd *abfd, bfd_reloc_code_real_type code) {
...
static reloc_howto_type *cris_reloc_name_lookup(bfd *abfd, const char *r_name) {
    if (r_name == NULL) {
        return NULL;
    }
...
static bool cris_info_to_howto_rela(bfd *abfd, arelent *cache_ptr, Elf_Internal_Rela *dst)
{
...
bfd_reloc_status_type cris_elf_pcrel_reloc(bfd *abfd, arelent *reloc_entry, asymbol *symbol, void *data, asection *input_section, bfd *output_bfd, char **error_message) {
    if (!output_bfd) {
...
static bool cris_elf_grok_prstatus(bfd *abfd, Elf_Internal_Note *note) {
    int offset;
    size_t size;
    uint32_t machine_type = bfd_get_mach(abfd);
    uint32_t desc_size = note->descsz;

    if (machine_type == bfd_mach_cris_v32 && desc_size == 202) {
        elf_tdata(abfd)->core->signal = bfd_get_16(abfd, note->descdata + 12);
        elf_tdata(abfd)->core->lwpid = bfd_get_32(abfd, note->descdata + 22);
        offset = 70;
        size = 128;
    } else if (desc_size == 214) {
        elf_tdata(abfd)->core->signal = bfd_get_16(abfd, note->descdata + 12);
        elf_tdata(abfd)->core->lwpid = bfd_get_32(abfd, note->descdata + 22);
        offset = 70;
        size = 140;
...
static bool cris_elf_grok_psinfo(bfd *abfd, Elf_Internal_Note *note) {
    if ((bfd_get_mach(abfd) == bfd_mach_cris_v32 && note->descsz == 124) ||
        (bfd_get_mach(abfd) != bfd_mach_cris_v32 && note->descsz == 124)) {
...
static struct bfd_hash_entry *elf_cris_link_hash_newfunc(struct bfd_hash_entry *entry, struct bfd_hash_table *table, const char *string) {
...
static struct bfd_link_hash_table *elf_cris_link_hash_table_create(bfd *abfd) {
    struct elf_cris_link_hash_table *ret;
    
    ret = (struct elf_cris_link_hash_table *)bfd_zmalloc(sizeof(struct elf_cris_link_hash_table));
    if (ret == NULL) {
        return NULL;
    }

    if (!_bfd_elf_link_hash_table_init(&ret->root, abfd, elf_cris_link_hash_newfunc, sizeof(struct elf_cris_link_hash_entry))) {
        free(ret);
        return NULL;
    }
...
static bfd_reloc_status_type cris_final_link_relocate(reloc_howto_type *howto, bfd *input_bfd, asection *input_section, bfd_byte *contents, Elf_Internal_Rela *rel, bfd_vma relocation) {
    ...
    if (r_type == R_CRIS_32_PLT_PCREL || r_type == R_CRIS_32_PCREL) {
        relocation -= 2;
    }
    ...
    status = _bfd_final_link_relocate(howto, input_bfd, input_section, contents, rel->r_offset, relocation, rel->r_addend);
    return status;
}

...
static int cris_elf_relocate_section(bfd *output_bfd ATTRIBUTE_UNUSED, struct bfd_link_info *info, 
    ...
    for (Elf_Internal_Rela *rel = relocs; rel < relend; rel++) {
        ...
        if (r_symndx < symtab_hdr->sh_info) {
            ...
            if (symname == NULL) symname = bfd_section_name(sec);
        } else {
            ...
            if (unresolved_reloc && (sec->owner->flags & DYNAMIC) != 0)
                relocation = 0;
            else if (h->root.type == bfd_link_hash_defined || h->root.type == bfd_link_hash_defweak) {
                if (bfd_link_pic(info) && ((!SYMBOLIC_BIND(info, h) && h->dynindx != -1) || !h->def_regular) && 
        ...
        if (sec != NULL && discarded_section(sec))
            RELOC_AGAINST_DISCARDED_SECTION(info, input_bfd, input_section, rel, 1, relend, R_CRIS_NONE, howto, 0, contents);
        ...
        case R_CRIS_32_GOTPLT:
            if (h != NULL && ((struct elf_cris_link_hash_entry *) h)->gotplt_offset != 0) {
                ...
                bfd_vma got_offset = ((struct elf_cris_link_hash_entry *) h)->gotplt_offset;
                BFD_ASSERT(h->dynindx != -1);
                BFD_ASSERT(sgotplt != NULL);
                relocation = got_offset;
                break;
            }
            if (h != NULL && (h->got.offset == (bfd_vma)-1 || (!bfd_link_pic(info) && 
                !(h->def_regular || (!h->def_dynamic && h->root.type == bfd_link_hash_undefweak)))) {
                _bfd_error_handler((h->got.offset == (bfd_vma)-1) ? 
        ...
        case R_CRIS_32_GOT: {
            ...
            if (h != NULL) {
                ...
                if (!elf_hash_table(info)->dynamic_sections_created ||
                    (!bfd_link_pic(info) && (h->def_regular || h->type == STT_FUNC || h->needs_plt)) ||
                    (bfd_link_pic(info) && (SYMBOLIC_BIND(info, h) || h->dynindx == -1) && h->def_regular)) {
                    if ((off & 1) != 0)
                        off &= ~1;
                    else {
            ...
            if (rel->r_addend != 0) {
                if (h == NULL)
                    _bfd_error_handler(_("%pB, section %pA: relocation %s with non-zero addend %" PRId64 " against local symbol"), 
        ...
        case R_CRIS_32_GOTREL:
            if (h != NULL && ELF_ST_VISIBILITY(h->other) == STV_DEFAULT && 
                !(!bfd_link_pic(info) && (h->def_regular || (!h->def_dynamic && h->root.type == bfd_link_hash_undefweak)))) {
                _bfd_error_handler(_("%pB, section %pA: relocation %s is not allowed for global symbol: `%s'"), 
                                   input_bfd, input_section, cris_elf_howto_table[r_type].name, symname);
                bfd_set_error(bfd_error_bad_value);
                return false;
            }
            if (sgot == NULL) {
                _bfd_error_handler(_("%pB, section %pA: relocation %s with no GOT created"), 
        ...
        case R_CRIS_16:
        case R_CRIS_32:
            if (bfd_link_pic(info) && !resolved_to_zero && r_symndx != STN_UNDEF && 
                (input_section->flags & SEC_ALLOC) != 0 && 
                ((r_type != R_CRIS_8_PCREL && r_type != R_CRIS_16_PCREL && r_type != R_CRIS_32_PCREL) || 
                 (!SYMBOLIC_BIND(info, h) || (h != NULL && !h->def_regular)))) {
                Elf_Internal_Rela outrel;
                bfd_byte *loc;
                bool skip = false, relocate = false;

                if (sreloc == NULL) {
                    sreloc = _bfd_elf_get_dynamic_reloc_section(dynobj, input_section, true);
                    if (sreloc == NULL) {
                        bfd_set_error(bfd_error_bad_value);
                        return false;
                    }
                ...
                if (outrel.r_offset == (bfd_vma)-1)
                    skip = true;
                else if (outrel.r_offset == (bfd_vma)-2 || (h != NULL && 
                         h->root.type == bfd_link_hash_undefweak && ELF_ST_VISIBILITY(h->other) != STV_DEFAULT))
                    skip = true, relocate = true;
                outrel.r_offset += (input_section->output_section->vma + input_section->output_offset);

                if (skip)
                    memset(&outrel, 0, sizeof outrel);
                else if (h != NULL && ((!SYMBOLIC_BIND(info, h) && h->dynindx != -1) || !h->def_regular)) {
                    BFD_ASSERT(h->dynindx != -1);
        ...
        case R_CRIS_32_DTPREL:
            if (h != NULL && (input_section->flags & SEC_ALLOC) != 0 && 
                ...
                _bfd_error_handler(h->root.type == bfd_link_hash_undefined ? 
                                   ...
                                   input_bfd, input_section, cris_elf_howto_table[r_type].name, symname != NULL && symname[0] != '\0' ? symname : _("[whose name is lost]"));
                bfd_set_error(bfd_error_bad_value);
                return false;
            }
            ...
            relocation -= elf_hash_table(info)->tls_sec == NULL ? 0 : elf_hash_table(info)->tls_sec->vma;
            break;
        ...
        case R_CRIS_32_TPREL:
            ...
            if (h != NULL && ELF_ST_VISIBILITY(h->other) == STV_DEFAULT && 
                ...
                _bfd_error_handler(_("%pB, section %pA: relocation %s is not allowed for symbol: `%s' which is defined outside the program, perhaps a declaration mixup?"), 
                                   input_bfd, input_section, cris_elf_howto_table[r_type].name, symname);
                bfd_set_error(bfd_error_bad_value);
                return false;
            }

            relocation -= elf_hash_table(info)->tls_sec == NULL ? 0 : (elf_hash_table(info)->tls_sec->vma + elf_hash_table(info)->tls_size);
            break;
        ...
        bfd_reloc_status_type r = cris_final_link_relocate(howto, input_bfd, input_section, contents, rel, relocation);
        ...
        if (r != bfd_reloc_ok) {
            ...
            case bfd_reloc_overflow:
                (*info->callbacks->reloc_overflow)(info, (h ? &h->root : NULL), symname, howto->name, 
            ...
            case bfd_reloc_undefined:
                (*info->callbacks->undefined_symbol)(info, symname, input_bfd, input_section, rel->r_offset, true);
                break;
...
static bool elf_cris_finish_dynamic_symbol(bfd *output_bfd, struct bfd_link_info *info, struct elf_link_hash_entry *h, Elf_Internal_Sym *sym) {
    struct elf_cris_link_hash_table *htab = elf_cris_hash_table(info);
    int plt_off1 = 2, plt_off2 = 10, plt_off3 = 16, plt_off3_value_bias = 4, plt_stub_offset = 8, plt_entry_size = PLT_ENTRY_SIZE;
    const bfd_byte *plt_entry = elf_cris_plt_entry, *plt_pic_entry = elf_cris_pic_plt_entry;
    ...
    if (h->plt.offset != (bfd_vma)-1) {
        asection *splt = htab->root.splt, *sgotplt = htab->root.sgotplt, *srela = htab->root.srelplt;
        ...
        BFD_ASSERT(h->dynindx != -1 && splt != NULL && sgotplt != NULL && (!has_gotplt || srela != NULL));
        bfd_vma got_offset = has_gotplt ? gotplt_offset : h->got.offset + htab->next_gotplt_entry;
        
        if (!bfd_link_pic(info)) {
            memcpy(splt->contents + h->plt.offset, plt_entry, plt_entry_size);
            bfd_put_32(output_bfd, got_base + got_offset, splt->contents + h->plt.offset + plt_off1);
        } else {
            memcpy(splt->contents + h->plt.offset, plt_pic_entry, plt_entry_size);
            bfd_put_32(output_bfd, got_offset, splt->contents + h->plt.offset + plt_off1);
        }

        if (has_gotplt) {
            bfd_vma rela_plt_index = (htab->dtpmod_refcount != 0) ? gotplt_offset / 4 - 2 - 3 : gotplt_offset / 4 - 3;
            bfd_put_32(output_bfd, rela_plt_index * sizeof(Elf32_External_Rela), splt->contents + h->plt.offset + plt_off2);
            bfd_put_32(output_bfd, -(h->plt.offset + plt_off3 + plt_off3_value_bias), splt->contents + h->plt.offset + plt_off3);
            bfd_put_32(output_bfd, splt->output_section->vma + splt->output_offset + h->plt.offset + plt_stub_offset, sgotplt->contents + got_offset);

            Elf_Internal_Rela rela = {.r_offset = sgotplt->output_section->vma + sgotplt->output_offset + got_offset, .r_info = ELF32_R_INFO(h->dynindx, R_CRIS_JUMP_SLOT), .r_addend = 0};
            bfd_elf32_swap_reloca_out(output_bfd, &rela, srela->contents + rela_plt_index * sizeof(Elf32_External_Rela));
        }
    ...
    if (h->got.offset != (bfd_vma)-1 && elf_cris_hash_entry(h)->reg_got_refcount > 0 && (bfd_link_pic(info) || (h->dynindx != -1 && h->plt.offset == (bfd_vma)-1 && !h->def_regular && h->root.type != bfd_link_hash_undefweak))) {
        asection *sgot = htab->root.sgot, *srela = htab->root.srelgot;
        BFD_ASSERT(sgot != NULL && srela != NULL);

        Elf_Internal_Rela rela = {.r_offset = sgot->output_section->vma + sgot->output_offset + (h->got.offset & ~(bfd_vma)1)};
        bfd_byte *where = sgot->contents + (h->got.offset & ~(bfd_vma)1);

        if (!elf_hash_table(info)->dynamic_sections_created || (bfd_link_pic(info) && (SYMBOLIC_BIND(info, h) || h->dynindx == -1) && h->def_regular)) {
        ...
        bfd_elf32_swap_reloca_out(output_bfd, &rela, srela->contents + srela->reloc_count++ * sizeof(Elf32_External_Rela));
    }

    if (h->needs_copy) {
        asection *s = (h->root.u.def.section == htab->root.sdynrelro) ? htab->root.sreldynrelro : htab->root.srelbss;
        BFD_ASSERT(h->dynindx != -1 && (h->root.type == bfd_link_hash_defined || h->root.type == bfd_link_hash_defweak));

        Elf_Internal_Rela rela = {.r_offset = h->root.u.def.value + h->root.u.def.section->output_section->vma + h->root.u.def.section->output_offset, .r_info = ELF32_R_INFO(h->dynindx, R_CRIS_COPY), .r_addend = 0};
        bfd_elf32_swap_reloca_out(output_bfd, &rela, s->contents + s->reloc_count++ * sizeof(Elf32_External_Rela));
    }
...
asection *cris_elf_gc_mark_hook(asection *sec, struct bfd_link_info *info, Elf_Internal_Rela *rel, struct elf_link_hash_entry *h, Elf_Internal_Sym *sym) {
    if (h != NULL) {
...
static bfd_vma cris_elf_plt_sym_val(bfd_vma i ATTRIBUTE_UNUSED, const asection *plt, const arelent *rel) {
    ...
    plt_entry_size = (bfd_get_mach(abfd) == bfd_mach_cris_v32 ? PLT_ENTRY_SIZE_V32 : PLT_ENTRY_SIZE);
    ...
    for (pltoffs = plt_entry_size; pltoffs < plt_sec_size; pltoffs += plt_entry_size) {
        ...
        if (!bfd_get_section_contents(abfd, (asection *)plt, gotoffs_raw, pltoffs + plt_entry_got_offset, sizeof(gotoffs_raw))) {
            return (bfd_vma)-1;
        }
...
bool elf_cris_adjust_gotplt_to_got(struct elf_cris_link_hash_entry *h, void *p) {
...
static bool elf_cris_adjust_dynamic_symbol(struct bfd_link_info *info, struct elf_link_hash_entry *h) {
    ...
    asection *s = (h->root.u.def.section->flags & SEC_READONLY) ? htab->root.sdynrelro : htab->root.sdynbss;
    asection *srel = (h->root.u.def.section->flags & SEC_READONLY) ? htab->root.sreldynrelro : htab->root.srelbss;
...
static void elf_cris_copy_indirect_symbol(struct bfd_link_info *info, struct elf_link_hash_entry *dir, struct elf_link_hash_entry *ind) {
...
static bool cris_elf_check_relocs(bfd *abfd, struct bfd_link_info *info, asection *sec, const Elf_Internal_Rela *relocs) {
    ...
    for (const Elf_Internal_Rela *rel = relocs; rel < rel_end; rel++) {
        ...
        h = (r_symndx < symtab_hdr->sh_info) ? NULL : sym_hashes[r_symndx - symtab_hdr->sh_info];
        if (h != NULL) {
        ...
        switch (r_type) {
            case R_CRIS_32_DTPREL:
                if (!(sec->flags & SEC_ALLOC)) continue;
                if (htab->dtpmod_refcount == 0) htab->next_gotplt_entry += 8;
            ...
            case R_CRIS_32_GOTPLT:
                if (dynobj == NULL) {
                    ...
                    if (bfd_get_mach(dynobj) == bfd_mach_cris_v10_v32) {
                        _bfd_error_handler(_("PIC relocation not allowed for v10/v32"), abfd, sec);
                        return false;
                    }
                }
                if (sgot == NULL) {
                    if (!_bfd_elf_create_got_section(dynobj, info)) return false;
                    sgot = elf_hash_table(info)->sgot;
                    srelgot = elf_hash_table(info)->srelgot;
                }
                if (local_got_refcounts == NULL) {
                    bfd_size_type amt = LGOT_ALLOC_NELTS_FOR(symtab_hdr->sh_info) + 1;
                    local_got_refcounts = ((bfd_signed_vma *)bfd_zalloc(abfd, amt * sizeof(bfd_signed_vma)));
                    if (local_got_refcounts == NULL) return false;
                    local_got_refcounts++;
        ...
        switch (r_type) {
            ...
            case R_CRIS_32_GD:
                if (bfd_link_pic(info)) {
                    _bfd_error_handler(_("relocation not valid in shared object"), abfd, sec, cris_elf_howto_table[r_type].name);
                }
                break;
            case R_CRIS_32_GD:
            case R_CRIS_16_GOT_GD:
        ...
        // Handle relocations that require GOT or PLT entries
        switch (r_type) {
            case R_CRIS_16_GOTPLT:
            case R_CRIS_32_GOTPLT:
                if (h != NULL) {
            ...
            case R_CRIS_32_GOT:
                if (h != NULL) {
                    if (h->got.refcount == 0 && h->dynindx == -1) {
                    ...
                    if (local_got_refcounts[r_symndx_lgot] == 0) {
                        sgot->size += got_element_size;
                        if (bfd_link_pic(info)) srelgot->size += sizeof(Elf32_External_Rela);
                    }
            ...
            case R_CRIS_16:
            case R_CRIS_32:
                if (bfd_link_pic(info) && (sec->flags & SEC_ALLOC) != 0 && (sec->flags & SEC_READONLY) != 0) {
                    _bfd_error_handler(_("relocation should not be used in shared object"), abfd, sec, cris_elf_howto_table[r_type].name);
                }
                if ((sec->flags & SEC_ALLOC) == 0) break;
                if (h != NULL) {
                    h->non_got_ref = 1;
                    if (ELF_ST_VISIBILITY(h->other) == STV_DEFAULT) h->plt.refcount++;
                }
                if (!bfd_link_pic(info) || (h != NULL && UNDEFWEAK_NO_DYNAMIC_RELOC(info, h))) break;
                if (sreloc == NULL) {
                    sreloc = _bfd_elf_make_dynamic_reloc_section(sec, dynobj, 2, abfd, true);
                    if (sreloc == NULL) return false;
                }
                if (sec->flags & SEC_READONLY) info->flags |= DF_TEXTREL;
            ...
            case R_CRIS_32_PCREL:
                ...
                if (!bfd_link_pic(info)) break;
                if ((sec->flags & SEC_ALLOC) == 0) break;
                if (h == NULL || ELF_ST_VISIBILITY(h->other) != STV_DEFAULT) break;
                if (SYMBOLIC_BIND(info, h) && h->root.type != bfd_link_hash_defweak && h->def_regular) break;
                if (sreloc == NULL) {
                    sreloc = _bfd_elf_make_dynamic_reloc_section(sec, dynobj, 2, abfd, true);
                    if (sreloc == NULL) return false;
                }
                sreloc->size += sizeof(Elf32_External_Rela);
                {
                  ...
                  for (p = eh->pcrel_relocs_copied; p != NULL; p = p->next) {
                      if (p->section == sec) break;
                  }
                  if (p == NULL) {
                      p = (struct elf_cris_pcrel_relocs_copied *)bfd_alloc(dynobj, sizeof(*p));
...
#define BFD_ASSERT(expr) ((void)(expr)) 
...
typedef struct bfd {
  ...
  // Other members not relevant for this context
} bfd;
...
#define SEC_LINKER_CREATED 0x1
#define SEC_HAS_CONTENTS 0x2
#define SEC_EXCLUDE 0x4
...
struct elf_cris_link_hash_table {
  struct {
    ...
    struct asection *srelgot;
    struct asection *sdynrelro;
  } root;
  int dtpmod_refcount;
};

struct elf_cris_link_hash_table *elf_cris_hash_table(bfd_link_info *info) {
  // Implementation specific to the project
  return NULL;
}

void elf_cris_link_hash_traverse(struct elf_cris_link_hash_table *htab, void func(), bfd_link_info *info) {
  // Implementation specific to the project
}

void elf_cris_adjust_gotplt_to_got() {}

void elf_cris_discard_excess_dso_dynamics() {}

void elf_cris_discard_excess_program_dynamics() {}

int _bfd_elf_add_dynamic_tags(bfd *output_bfd, bfd_link_info *info, bool relocs) {
  // Implementation specific to the project
  return 0;
}

const char *bfd_section_name(asection *s) {
  // Implementation specific to the project
  return NULL;
}

asection *bfd_get_linker_section(bfd *dynobj, const char *name) {
  // Implementation specific to the project
  return NULL;
}

bool bfd_link_executable(bfd_link_info *info) {
  // Implementation specific to the project
  return false;
}

bool bfd_link_pic(bfd_link_info *info) {
  // Implementation specific to the project
  return false;
}
...
size_t sizeof_Elf32_External_Rela() {
  return sizeof(uint32_t) * 3;
}

void *bfd_zalloc(bfd *dynobj, size_t size) {
  return calloc(1, size);
}

bool elf_cris_late_size_sections(bfd *output_bfd ATTRIBUTE_UNUSED, bfd_link_info *info) {
  struct elf_cris_link_hash_table *htab = elf_cris_hash_table(info);
  if (!htab) {
    return false;
  }
  ...
  for (asection *s = dynobj->sections; s; s = s->next) {
    ...
    if (strcmp(name, ".got.plt") == 0) {
      ...
      if (strcmp(name, ".rela.got") == 0 && htab->dtpmod_refcount != 0 && bfd_link_pic(info)) {
        s->size += sizeof_Elf32_External_Rela();
      }
      if (s->size != 0 && strcmp(name, ".rela.plt") != 0) {
        relocs = true;
        s->flags |= SEC_HAS_CONTENTS;
        s->alloced = true;
      }
    } else if (s->size != 0 || strcmp(name, ".plt") != 0) {
      if (s->size == 0) {
        s->flags |= SEC_EXCLUDE;
        continue;
      }
...
static bool elf_cris_discard_excess_dso_dynamics(struct elf_cris_link_hash_entry *h, void *inf) {
    ...
    if (h->root.def_regular && (h->root.forced_local || SYMBOLIC_BIND(info, &h->root))) {
        while (s) {
            asection *sreloc = _bfd_elf_get_dynamic_reloc_section(elf_hash_table(info)->dynobj, s->section, true);
    ...
    while (s) {
        if ((s->section->flags & SEC_READONLY) != 0) {
            _bfd_error_handler(_("%pB, section `%pA', to symbol `%s': relocation %s should not be used in a shared object; recompile with -fPIC"), s->section->owner, s->section, h->root.root.root.string, cris_elf_howto_table[s->r_type].name);
...
static bool elf_cris_discard_excess_program_dynamics(struct elf_cris_link_hash_entry *h, void *inf) {
    ...
    if (!h->root.def_dynamic || h->root.plt.refcount > 0) {
        ...
        if (h->reg_got_refcount > 0 && elf_hash_table(info)->dynamic_sections_created) {
            bfd *dynobj = elf_hash_table(info)->dynobj;
            asection *srelgot = elf_hash_table(info)->srelgot;
        ...
        if (!(info->export_dynamic || (h->root.type != STT_FUNC && info->dynamic_data)) && 
            h->root.dynindx != -1 && 
            !h->root.dynamic && 
            !h->root.def_dynamic && 
            !h->root.ref_dynamic) {
            ...
            h->root.dynindx = -1;
            _bfd_elf_strtab_delref(elf_hash_table(info)->dynstr, h->root.dynstr_index);
        }
    }
...
static bool cris_elf_object_p(bfd *abfd) {
    ...
    bool underscore_flag = elf_elfheader(abfd)->e_flags & EF_CRIS_UNDERSCORE;

    return underscore_flag ? (leading_char == '_') : (leading_char == 0);
}

/* Mark presence or absence of leading underscore.  Set machine type
   flags from mach type.  */

bool cris_elf_final_write_processing(bfd *abfd) {
    if (abfd == NULL) {
        return false;  // Error handling for null pointer
    }
    ...
    if (mach == bfd_mach_cris_v0_v10) {
        ...
        _bfd_abort(__FILE__, __LINE__, "unexpected machine number");
        return false;  // Error handling for unexpected case
    }
...
static bool cris_elf_print_private_bfd_data(bfd *abfd, void *ptr) {
    ...
    fprintf(file, "private flags = %lx:", flags);

    if (flags & EF_CRIS_UNDERSCORE) {
        fprintf(file, " [symbols have a _ prefix]");
    }
    if ((flags & EF_CRIS_VARIANT_MASK) == EF_CRIS_VARIANT_COMMON_V10_V32) {
        fprintf(file, " [v10 and v32]");
    }
...
static bool cris_elf_merge_private_bfd_data(bfd *ibfd, struct bfd_link_info *info) {
    ...
    if (!_bfd_generic_verify_endian_match(ibfd, info)) return false;

    if (bfd_get_flavour(ibfd) != bfd_target_elf_flavour || bfd_get_flavour(obfd) != bfd_target_elf_flavour) return true;

    int imach = bfd_get_mach(ibfd);
    if (!elf_flags_init(obfd)) {
        elf_flags_init(obfd) = true;
        if (!bfd_set_arch_mach(obfd, bfd_arch_cris, imach)) return false;
    }

    if (bfd_get_symbol_leading_char(ibfd) != bfd_get_symbol_leading_char(obfd)) {
        _bfd_error_handler(bfd_get_symbol_leading_char(ibfd) == '_' ? _("%pB: uses _-prefixed symbols, but writing file with non-prefixed symbols") : _("%pB: uses non-prefixed symbols, but writing file with _-prefixed symbols"), ibfd);
        bfd_set_error(bfd_error_bad_value);
        return false;
    }
    ...
    if (imach != omach) {
        if ((imach == bfd_mach_cris_v32 && omach != bfd_mach_cris_v10_v32) || (omach == bfd_mach_cris_v32 && imach != bfd_mach_cris_v10_v32)) {
            _bfd_error_handler(imach == bfd_mach_cris_v32 ? _("%pB contains CRIS v32 code, incompatible with previous objects") : _("%pB contains non-CRIS-v32 code, incompatible with previous objects"), ibfd);
            bfd_set_error(bfd_error_bad_value);
            return false;
        }
        if (omach == bfd_mach_cris_v10_v32 && !bfd_set_arch_mach(obfd, bfd_arch_cris, imach)) return false;
    }
...
bool cris_elf_copy_private_bfd_data(bfd *ibfd, bfd *obfd) {
...
static enum elf_reloc_type_class elf_cris_reloc_type_class(const struct bfd_link_info *info, const asection *rel_sec, const Elf_Internal_Rela *rela) {
    switch (ELF32_R_TYPE(rela->r_info)) {
        case R_CRIS_RELATIVE: return reloc_class_relative;
        case R_CRIS_JUMP_SLOT: return reloc_class_plt;
        case R_CRIS_COPY: return reloc_class_copy;
...
static bfd_vma elf_cris_got_elt_size(bfd *abfd ATTRIBUTE_UNUSED,
    ...
    if (hr == NULL) {
        ...
        int reg_ndx = LGOT_REG_NDX(symndx);
        int dtp_ndx = LGOT_DTP_NDX(symndx);
        int tprel_ndx = LGOT_TPREL_NDX(symndx);

        if (local_got_refcounts[reg_ndx] > 0) {
            if (local_got_refcounts[dtp_ndx] != 0 || local_got_refcounts[tprel_ndx] != 0) {
                return 0;
            }
            return 4;
        }
        ...
        bfd_vma eltsiz = 0;
        if (local_got_refcounts[dtp_ndx] > 0) {
            eltsiz += 8;
        }
        if (local_got_refcounts[tprel_ndx] > 0) {
            eltsiz += 4;
        }

        return eltsiz ? eltsiz : 0;
    } else {
