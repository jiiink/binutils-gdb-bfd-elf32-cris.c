...
{
  size_t i;
  const size_t num_entries = sizeof (cris_reloc_map) / sizeof (cris_reloc_map[0]);
...
cris_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  table_size = sizeof (cris_elf_howto_table) / sizeof (cris_elf_howto_table[0]);
...
cris_elf_grok_psinfo (bfd *abfd, Elf_Internal_Note *note)
{
  ...
  if (!note->descdata || !abfd || !elf_tdata(abfd) || !elf_tdata(abfd)->core) {
    return false;
  }
			  ...
			  bfd_vma relocation)
{
  ...
  if (!howto || !input_bfd || !input_section || !contents || !rel)
				...
				Elf_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
      if (!process_plt_entry(output_bfd, info, h, htab, plt_off1, plt_off2, plt_off3,
                             plt_off3_value_bias, plt_stub_offset, plt_entry_size,
                             plt_entry, plt_pic_entry))
        return false;
    }
  ...
  if (h->got.offset != (bfd_vma) -1 &&
      ...
      (bfd_link_pic (info) ||
       (h->dynindx != -1 && h->plt.offset == (bfd_vma) -1 &&
  ...
  if (h->needs_copy)
    {
      if (!process_copy_reloc(output_bfd, info, h, htab))
        return false;
    }
...
process_plt_entry(bfd *output_bfd, struct bfd_link_info *info,
                  struct elf_link_hash_entry *h,
                  struct elf_cris_link_hash_table *htab,
                  int plt_off1, int plt_off2, int plt_off3,
                  int plt_off3_value_bias, int plt_stub_offset,
                  int plt_entry_size, const bfd_byte *plt_entry,
                  const bfd_byte *plt_pic_entry)
{
  asection *splt, *sgotplt, *srela;
  bfd_vma got_base, gotplt_offset, got_offset, rela_plt_index;
  ...
  rela_plt_index = (htab->dtpmod_refcount != 0
                   ? gotplt_offset/4 - 2 - 3 : gotplt_offset/4 - 3);

  got_offset = (has_gotplt
               ? gotplt_offset
  ...
  if (has_gotplt)
    {
      ...
      bfd_put_32 (output_bfd,
                  (splt->output_section->vma + splt->output_offset +
                   h->plt.offset + plt_stub_offset),
                  sgotplt->contents + got_offset);

      rela.r_offset = (sgotplt->output_section->vma + sgotplt->output_offset +
                       got_offset);
...
static bool
process_copy_reloc(bfd *output_bfd, struct bfd_link_info *info,
                   struct elf_link_hash_entry *h,
                   struct elf_cris_link_hash_table *htab)
{
  asection *s;
				  ...
				  struct bfd_link_info *info)
{
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      if (splt->size > 0)
	{
	  ...
	  if (is_v32)
		...
		{
		  bfd_vma got_base = sgot->output_section->vma + sgot->output_offset;
		  memcpy (splt->contents, elf_cris_plt0_entry, PLT_ENTRY_SIZE);
		  bfd_put_32 (output_bfd, got_base + 4, splt->contents + 6);
		  bfd_put_32 (output_bfd, got_base + 8, splt->contents + 14);
		}
  ...
  if (sgot->size > 0)
    {
      bfd_vma dynamic_addr = (sdyn != NULL) ? 
        (sdyn->output_section->vma + sdyn->output_offset) : 0;
				...
				struct elf_link_hash_entry *h)
{
  ...
  plt_entry_size = (bfd_get_mach (dynobj) == bfd_mach_cris_v32) 
		       ...
		       const Elf_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      else
	{
      ...
      if (!cris_elf_handle_setup_relocs (abfd, info, sec, r_type, &dynobj, &sgot, &srelgot, &local_got_refcounts, htab, symtab_hdr))
	return false;

      if (!cris_elf_validate_reloc_for_shared_lib (abfd, sec, info, r_type))
	return false;

      if (!cris_elf_process_relocation (abfd, info, sec, rel, h, r_symndx, r_type, got_element_size, r_symndx_lgot, &dynobj, &sgot, &srelgot, &sreloc, local_got_refcounts))
	return false;
    }
...
static bool
cris_elf_handle_setup_relocs (bfd *abfd, struct bfd_link_info *info, asection *sec, 
			      enum elf_cris_reloc_type r_type, bfd **dynobj,
			      asection **sgot, asection **srelgot,
			      bfd_signed_vma **local_got_refcounts,
			      struct elf_cris_link_hash_table *htab,
			      Elf_Internal_Shdr *symtab_hdr)
{
  bool needs_dynamic_sections = false;
  ...
  switch (r_type)
    ...
    case R_CRIS_16_GOTPLT:
    case R_CRIS_32_GOTPLT:
      needs_dynamic_sections = true;
      break;
    default:
      break;
    }

  if (!needs_dynamic_sections)
    return true;
...
static bool
cris_elf_validate_reloc_for_shared_lib (bfd *abfd, asection *sec,
					struct bfd_link_info *info,
					enum elf_cris_reloc_type r_type)
{
  switch (r_type)
...
static bool
cris_elf_process_relocation (bfd *abfd, struct bfd_link_info *info,
			     ...
			     asection **sreloc, bfd_signed_vma *local_got_refcounts)
{
  switch (r_type)
    {
    case R_CRIS_32_GD:
  ...
  switch (r_type)
    ...
    case R_CRIS_16_GOTPLT:
    case R_CRIS_32_GOTPLT:
      return cris_elf_handle_gotplt_reloc (h, r_symndx_lgot, got_element_size, *sgot, *srelgot, local_got_refcounts, info);
    ...
    case R_CRIS_16_GOT:
    case R_CRIS_32_GOT:
      return cris_elf_handle_got_reloc (h, r_symndx, r_symndx_lgot, r_type, got_element_size, *sgot, *srelgot, local_got_refcounts, info);
    ...
    case R_CRIS_32_PLT_GOTREL:
      local_got_refcounts[-1]++;
    case R_CRIS_32_PLT_PCREL:
      return cris_elf_handle_plt_reloc (h);

    case R_CRIS_8:
    case R_CRIS_16:
    case R_CRIS_32:
      return cris_elf_handle_absolute_reloc (abfd, info, sec, h, r_type, dynobj, sreloc);

    case R_CRIS_8_PCREL:
    case R_CRIS_16_PCREL:
    case R_CRIS_32_PCREL:
      return cris_elf_handle_pcrel_reloc (abfd, info, sec, h, r_type, dynobj, sreloc);
...
static bool
cris_elf_handle_gotplt_reloc (struct elf_link_hash_entry *h,
			      unsigned long r_symndx_lgot,
			      bfd_signed_vma got_element_size,
			      asection *sgot, asection *srelgot,
			      bfd_signed_vma *local_got_refcounts,
  ...
  if (h != NULL)
    {
      ...
      return cris_elf_handle_got_reloc (h, 0, r_symndx_lgot, R_CRIS_16_GOT, got_element_size, sgot, srelgot, local_got_refcounts, info);
    }
  return cris_elf_handle_got_reloc (NULL, 0, r_symndx_lgot, R_CRIS_16_GOT, got_element_size, sgot, srelgot, local_got_refcounts, info);
}

static bool
cris_elf_handle_got_reloc (struct elf_link_hash_entry *h,
			   unsigned long r_symndx,
			   unsigned long r_symndx_lgot,
			   enum elf_cris_reloc_type r_type,
			   bfd_signed_vma got_element_size,
			   ...
			   struct bfd_link_info *info)
{
  if (h != NULL)
      ...
      if (h->got.refcount == 0 && h->dynindx == -1)
	{
      ...
      return cris_elf_update_hash_entry_got_counts (h, r_type, got_element_size, sgot, srelgot);
    }
  else
    {
...
static bool
cris_elf_update_hash_entry_got_counts (struct elf_link_hash_entry *h,
					enum elf_cris_reloc_type r_type,
					bfd_signed_vma got_element_size,
					asection *sgot, asection *srelgot)
{
  struct elf_cris_link_hash_entry *cris_h = elf_cris_hash_entry (h);
  
  switch (r_type)
    {
    case R_CRIS_16_GOT:
    case R_CRIS_32_GOT:
      if (cris_h->reg_got_refcount == 0)
	{
	  sgot->size += got_element_size;
	  srelgot->size += sizeof (Elf32_External_Rela);
	}
      cris_h->reg_got_refcount++;
      break;

    case R_CRIS_32_GD:
    case R_CRIS_16_GOT_GD:
    case R_CRIS_32_GOT_GD:
      if (cris_h->dtp_refcount == 0)
	{
	  sgot->size += got_element_size;
	  srelgot->size += sizeof (Elf32_External_Rela);
	}
    ...
    case R_CRIS_32_GOT_TPREL:
    case R_CRIS_16_GOT_TPREL:
      if (cris_h->tprel_refcount == 0)
	{
	  sgot->size += got_element_size;
	  srelgot->size += sizeof (Elf32_External_Rela);
	}
      cris_h->tprel_refcount++;
      break;
...
static bool
cris_elf_handle_plt_reloc (struct elf_link_hash_entry *h)
{
  if (h == NULL)
    return true;
...
static bool
cris_elf_handle_absolute_reloc (bfd *abfd, struct bfd_link_info *info,
				asection *sec, struct elf_link_hash_entry *h,
				enum elf_cris_reloc_type r_type,
				bfd **dynobj, asection **sreloc)
{
  if (bfd_link_pic (info)
  ...
  return cris_elf_create_dynamic_reloc_section (sec, *dynobj, sreloc, abfd, info);
}

static bool
cris_elf_handle_pcrel_reloc (bfd *abfd, struct bfd_link_info *info,
			     asection *sec, struct elf_link_hash_entry *h,
			     enum elf_cris_reloc_type r_type,
			     bfd **dynobj, asection **sreloc)
{
  ...
  if (!cris_elf_create_dynamic_reloc_section (sec, *dynobj, sreloc, abfd, info))
    return false;

  return cris_elf_record_pcrel_reloc (h, sec, r_type, *dynobj);
}

static bool
cris_elf_create_dynamic_reloc_section (asection *sec, bfd *dynobj,
					asection **sreloc, bfd *abfd,
					struct bfd_link_info *info)
{
  if (*sreloc == NULL)
    {
...
static bool
cris_elf_record_pcrel_reloc (struct elf_link_hash_entry *h, asection *sec,
			     enum elf_cris_reloc_type r_type, bfd *dynobj)
{
				      ...
				      void * inf)
{
  ...
  if (h->root.def_regular
      ...
      for (s = h->pcrel_relocs_copied; s != NULL; s = s->next)
	{
	  ...
	  asection *sreloc = _bfd_elf_get_dynamic_reloc_section (dynobj,
						  s->section,
						  true);
	  if (sreloc && s->count > 0)
  ...
  for (s = h->pcrel_relocs_copied; s != NULL; s = s->next)
	...
	{
	  const char *symbol_name = (h->root.root.root.string) ? 
	    h->root.root.root.string : "<unknown>";
	  const char *howto_name = (s->r_type < ARRAY_SIZE(cris_elf_howto_table)) ?
	    cris_elf_howto_table[s->r_type].name : "<unknown>";
					  ...
					  void * inf)
{
  ...
  bool should_export_all = info->export_dynamic || 
                          (h->root.type != STT_FUNC && info->dynamic_data);
  bool is_eligible_for_removal = h->root.dynindx != -1 &&
                                !h->root.dynamic &&
                                !h->root.def_dynamic &&
                                !h->root.ref_dynamic;

  if (!should_export_all && is_eligible_for_removal) {
    h->root.dynindx = -1;
...
cris_elf_object_p (bfd *abfd)
{
  ...
  expected_leading_char = (ehdr->e_flags & EF_CRIS_UNDERSCORE) ? '_' : 0;
...
cris_elf_set_mach_from_flags (bfd *abfd, unsigned long flags)
{
  ...
  static const flag_mach_map_t flag_mach_map[] = {
    { EF_CRIS_VARIANT_ANY_V0_V10, bfd_mach_cris_v0_v10 },
    { EF_CRIS_VARIANT_V32, bfd_mach_cris_v32 },
    { EF_CRIS_VARIANT_COMMON_V10_V32, bfd_mach_cris_v10_v32 }
  };
  ...
  for (size_t i = 0; i < sizeof(flag_mach_map) / sizeof(flag_mach_map[0]); i++) {
    if (variant == flag_mach_map[i].flag) {
      bfd_default_set_arch_mach (abfd, bfd_arch_cris, flag_mach_map[i].mach);
      return true;
    }
  }
...
cris_elf_merge_private_bfd_data (bfd *ibfd, struct bfd_link_info *info)
{
  ...
  if (bfd_get_symbol_leading_char(ibfd) != bfd_get_symbol_leading_char(obfd)) {
    const char *msg = (bfd_get_symbol_leading_char(ibfd) == '_') ?
  ...
  if ((imach == bfd_mach_cris_v32 && omach != bfd_mach_cris_v10_v32) ||
      (omach == bfd_mach_cris_v32 && imach != bfd_mach_cris_v10_v32)) {
    const char *msg = (imach == bfd_mach_cris_v32) ?
